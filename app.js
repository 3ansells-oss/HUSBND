...
/// Lets the person who currently owns an unanswered proposal fix the title,
/// notes, or BJ value on it before the other person responds — for typos,
/// or just changing your mind before anyone's acted on it.
async function editChore(chore, { title, notes, bjValue }) {
  await updateDoc(doc(state.db, "chores", chore.id), {
    title: title.trim(),
    notes: notes.trim(),
    bjValue,
...
function canEditChore(chore, role) {
  // You can only edit your own outstanding, unanswered proposal — same window
  // during which you're allowed to withdraw it.
  return chore.status === "proposed" && chore.proposedBy === role;
}
...
function renderDetailSheet() {
  ...
  if (state.detailEditing && !canEditChore(chore, role)) {
    // The underlying chore changed under us (e.g. the other person just
    // responded) — editing no longer makes sense, so drop back to the normal view.
    state.detailEditing = false;
  }

  if (state.detailEditing) {
    renderDetailEditForm(chore);
    return;
  }
  ...
  if (awaitsResponse(chore, role)) {
    ...
    actions += `
      ...
      <div class="actions">
        <button class="btn btn-secondary" id="detail-edit-start">Edit Chore</button>
  ...
  bind("detail-edit-start", () => {
    state.editTitle = chore.title;
    state.editNotes = chore.notes;
    state.editValue = chore.bjValue;
    state.detailEditing = true;
    renderDetailSheet();
  });
}

function renderDetailEditForm(chore) {
  $("#detail-content").innerHTML = `
    <h3>Edit Chore</h3>
    ...
    <div class="stepper">
      <button type="button" id="edit-value-minus">−</button>
      <div class="stepper-value"><span id="edit-value-display">${state.editValue}</span> BJ${state.editValue === 1 ? "" : "s"}</div>
      <button type="button" id="edit-value-plus">+</button>
    </div>
    <div class="actions">
  ...
  `;
  // Set values via the DOM rather than interpolating into the HTML string,
  // so we don't have to worry about quote/HTML-escaping inside attributes.
  $("#edit-title").value = state.editTitle;
  $("#edit-notes").value = state.editNotes;

  const captureFields = () => {
    state.editTitle = $("#edit-title").value;
    state.editNotes = $("#edit-notes").value;
  };
  ...
  bind("edit-value-minus", () => {
    captureFields();
    state.editValue = Math.max(1, state.editValue - 1);
    renderDetailSheet();
  });
  bind("edit-value-plus", () => {
    captureFields();
    state.editValue = Math.min(100, state.editValue + 1);
    renderDetailSheet();
  });
  bind("edit-cancel", () => {
    state.detailEditing = false;
  ...
  bind("edit-save", () => {
    ...
    const bjValue = state.editValue;
    saveEditedChore(chore, { title, notes, bjValue });
  });
}

async function saveEditedChore(chore, fields) {
  // Deliberately not routed through runDetailAction: on failure we want to
  // stay in edit mode (with whatever the user typed still in the fields)
  // rather than silently drop back to the read-only view.
  try {
    await editChore(chore, fields);
    state.detailEditing = false;
    closeDetailSheet();
  } catch (e) {
    console.error(e);
